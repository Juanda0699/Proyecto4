...
# ---
# 
# * Estudiante: **Juan David Caicedo Abadía**
# * Carné: **B71365**
# * Grupo: **1**
# 
# ---
...
ipython nbconvert --to script "p4.ipynb"
...
# Se modifica la función modulador a 16-QAM
def modulador(bits, fc, mpp):
    '''Un método que simula el esquema de
    modulación digital BPSK.
    ...
    # Ordenar los bits a una matriz (N,4)
    newbits = bits.reshape(-1, 4)

    # 2. Construyendo un periodo de la señal portadora s(t)
    Tc = 1 / fc  # periodo [s]

    # mpp: muestras por período por cada 2 bits
    ...
    for b1, b2, b3, b4 in newbits:
        if b1 == 0 and b2 == 0:
            senal_Tx[i*mpp: (i+2)*mpp] = portadoracos * -3
            moduladora[i*mpp: (i+2)*mpp] = -3
        if b1 == 0 and b2 == 1:
            senal_Tx[i*mpp: (i+2)*mpp] = portadoracos * -1
        ...
        if b3 == 0 and b4 == 0:
            senal_Tx[(i + 2)*mpp: (i + 4)*mpp] = portadorasin*3
            moduladora[(i + 2)*mpp: (i + 4)*mpp] = 3
        if b3 == 0 and b4 == 1:
            senal_Tx[(i + 2)*mpp: (i + 4)*mpp] = portadorasin*1
    ...
    # 5. Calcular la potencia promedio de la señal modulada
    P_senal_Tx = (1 / (N*Tc)) * np.trapz(pow(senal_Tx, 2), t_simulacion)
    return senal_Tx, P_senal_Tx, portadorasin, portadoracos, moduladora
...
def demodulador(senal_Rx, portadorasin, portadoracos, mpp):
    ...
    for i in range(0, N, 4):
        ...
        productocos = (senal_Rx[i*mpp:(i+2)*mpp] * portadoracos)
        Epcos = np.sum(productocos)

        # Reconstruyendo la parte coseno de la señal demodulada
        senal_demodulada[i*mpp:(i+2)*mpp] = productocos

        # Producto interno de dos funciones para la portadora seno
        productosin = (senal_Rx[(i+2)*mpp:(i+4)*mpp] * portadorasin)
        Epsin = np.sum(productosin)

        # Reconstruyendo la parte seno de la señal demodulada
        senal_demodulada[(i+2)*mpp:(i+4)*mpp] = productosin
        ...
        if Epcos > 0 and Epcos < 40:
            bits_Rx[i] = 1
            bits_Rx[i+1] = 1

        if Epcos > -40 and Epcos < 0:
            bits_Rx[i] = 0
            bits_Rx[i+1] = 1
        ...
        if Epsin > 0 and Epsin < 40:
            bits_Rx[i+2] = 0
            bits_Rx[i+3] = 1

        if Epsin > -40 and Epsin < 0:
            bits_Rx[i+2] = 1
            bits_Rx[i+3] = 1
...
# 3. Modular la cadena de bits usando el esquema BPSK
senal_Tx, Pm, portadorasin, portadoracos, moduladora = modulador(bits_Tx, fc, mpp)

# 4. Se transmite la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_Tx, Pm, SNR)

# 5. Se desmodula la señal recibida del canal
bits_Rx, senal_demodulada = demodulador(senal_Rx, portadorasin, portadoracos, mpp)
...
Esenal_Tx = np.average(senal_Tx)

print("Promedio estadístico senal Tx: {:0.9f}".format(Esenal_Tx))
...
# Al integrar de infinito a cero, por propiedades se utiliza T y no 2T
Asenal_Tx = (1 / T) * np.trapz(senal_Tx, t_simulacion)

print("Promedio temporal seal Tx: {:0.8f}".format(Asenal_Tx))

# 3. Cálculo de porcentaje de error entre el promedio estadístico y temporal
Perror = (abs(Asenal_Tx-Esenal_Tx)/Asenal_Tx)*100

print("Porcentaje de error entre promedio de señales: {:0.8f}%".format(Perror))
print("Se concluye que la señal es un proceso:")
if (Perror < 0.01):
    print("Ergódico, es decir estacionario.")

else:
    print(" No ergódico, es decir no estacionario.")
...
# Gráfica densidad espectral de potencia de Tx
plt.plot(f, 2.0/Nm * np.power(np.abs(senal_f[0:Nm//2]), 2))
plt.xlim(0, 10000)
plt.title("Densidad Espectral de Potencia de señal Tx")
